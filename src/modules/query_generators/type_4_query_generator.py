...
from modules.filters import generate as generate_filter
...
def generate():
    ...
    dimension_phrase = f"por {dimension.iloc[1]}"
    filter_phrase, filter_api = generate_filter()

    # Seleccionar un tipo de rango de fecha aleatorio
    date_phrase, date_range_api_query = generate_date_ranges()

    natural_language_query = f"{initial_phrase} {metric_phrase.lower()} {dimension_phrase} {filter_phrase} {date_phrase}."
    return (natural_language_query, {"metrics": [metric.iloc[0]], "dimensions": [dimension.iloc[0]], "filters":[filter_api], "dateRanges": [date_range_api_query]})
